...
function EspObject:Render()
    ...
    if visible.armorBar.Visible then
        ...
        local armor, maxArmor = self.interface.getArmor(self.player), 100
        local percent = math.clamp(armor / maxArmor, 0, 1)

        local armorBar = visible.armorBar
        armorBar.To = barTo
        armorBar.From = lerp2(barTo, barFrom, percent)
        armorBar.Color = Color3.fromRGB(0, 170, 255)
    ...
    if visible.armorText.Visible then
        ...
        armorText.Position = lerp2(barTo, barFrom, armor/100) - armorText.TextBounds*0.5 + ARMOR_TEXT_OFFSET
    end
...
local EspInterface = {
	...
	teamSettings = {
		enemy = {
            ...
            armorText = false,
            armorTextColor = { Color3.new(0,170/255,1), 1 },
...
function EspInterface.getArmor(player)
    ...
    if character then
        ...
        if bodyEffects then
            local armor = findFirstChild(bodyEffects, 'Armor')
